use fhe_vdaf_2::aggregator::run_aggregator;
use fhe_vdaf_2::artifacts::RuntimeLayout;
use fhe_vdaf_2::config::{AGGREGATORS_NUM, DEFAULT_RUNTIME_DIR};
...
fn main() -> Result<()> {
    ...
    if let Some(id) = args.next() {
        let aggregator_idx = id.parse::<usize>().expect("aggregator id must be an integer");
        let count = run_aggregator(&layout, aggregator_idx)?;
        println!("aggregator {aggregator_idx} processed {count} shard(s)");
    } else {
