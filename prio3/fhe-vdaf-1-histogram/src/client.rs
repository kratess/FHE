...
use crate::config::{AGGREGATORS_NUM, RNG_SEED, sample_client_values};
use crate::encoding::encode_bucket;
...
pub fn run_clients(layout: &RuntimeLayout, clients: &[Client]) -> Result<usize> {
    ...
    for (client_idx, client) in clients.iter().enumerate() {
        ...
        for (input_idx, (bucket, ct)) in client
            .values
            .iter()
            .copied()
            ...
            .enumerate()
        {
            expected_total += bucket;
            let aggregator_idx = rng.gen_range(0..AGGREGATORS_NUM);
            ct.save_to_file(
                &ctx,
                layout.client_ciphertext_path(aggregator_idx, client_idx, input_idx),
            )?;
        }
    }
