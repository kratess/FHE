...
impl Aggregator {
    ...
    fn validity_mask(ctx: &BgvContext, ct: &Ciphertext<usize>) -> Result<Ciphertext<usize>> {
        ...
        let bit_err_sum = Self::sum_all_slots_slot0(ctx, &bit_err_per_slot)?;

        let mut one_slot = vec![0usize; TOTAL_SLOTS];
        one_slot[0] = 1;
        let one_slot_pt = ctx.make_plaintext(&one_slot)?;
        let bucket_sum = Self::sum_all_slots_slot0(ctx, ct)?;
        let sum_minus_one = ctx.eval_sub_plain(&bucket_sum, &one_slot_pt)?;
        let sum_err = ctx.eval_mul(&bucket_sum, &sum_minus_one)?;
        let err_sum = ctx.eval_add(&bit_err_sum, &sum_err)?;
