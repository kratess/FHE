...
public:
    static CCParams<CryptoContextBGVRNS> GetParams(int depth, int ringDim) {
    ...
    static ContextData* GetContext(const std::string& opName, int depth, int ringDim) {
        ...
        if (!currentContext || currentParams != key) {
            ...
            std::cout << "Generating BGV context for Depth=" << depth << ", RingDim=" << ringDim << " (Slots=" << slots << ")" << std::endl;
            auto data = std::make_unique<ContextData>();
            data->depth = depth;
            data->slots = slots;

            CCParams<CryptoContextBGVRNS> parameters = GetParams(depth, ringDim);
